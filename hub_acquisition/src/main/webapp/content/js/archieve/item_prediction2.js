...
//			height:tool_max_h,
	...
	$( ".switch_tool input[type=checkbox]" ).on( "click", function(){
		...
		if (n==1){
			...
			var _length = Math.floor(item_prediction_data.length*0.75);
...
//				console.log(item_prediction_data[i]);
				
//				console.log(d3.select(".link_pred[data-item='"+item_prediction_data[i].i+"']"));
				
				d3.select(".link_pred[data-item_id='"+item_prediction_data[i].i+"']")
					.style("opacity",0)
					;
				d3.select(".node_g[data-item_id='"+item_prediction_data[i].i+"']")
					.style("opacity",0)
					;
				d3.select(".text_node_pred[data-item_id='"+item_prediction_data[i].i+"']")
					.style("opacity",0)
					;
...
//	console.log(data);
	...
	d3.select(".pred_graph_svg").select("*").remove();
...
//		return 50;
	...
	var simulation = d3.forceSimulation()
						.alphaDecay(0.2)
						.force("center", d3.forceCenter( (width/2), (height/2-su) )) 
...
//    	console.log(data);		
    	...
    	nodes = prediction_graph_data.nodes;
 	    
 	    links = prediction_graph_data.links;
...
//		 	        	
//		 	        	console.log(item_prediction_data.findIndex(({ i }) => i === d.target));
//		 	        	
 	    
//		 	        	
//		 	        	return d;
//		 	        }))
		 	        .data(links.filter(d=>(((item_prediction_data.findIndex(({ i }) => i === d.target)) <15  )?true:false )))
		 	        .enter()
		 	        .append("line")
...
//		 	        	return 11.75+1*(1-link_size_funct(d.value));
 	    ...
 	    node = pred_graph_svg.selectAll(".node_g")
		 	        .data(nodes.filter(d=>(((item_prediction_data.findIndex(({ i }) => i === d.itemId)) <15  )?true:false )))
		 	        .enter()
		 	        .append("g")
		 	        .attr("class", "node_g")
...
//				single_pred_node_click(d3.select(this)); 
 	    ...
 	    node.append("circle")
 	        ...
 	        .attr("r", function(d) {
 		    	return node_size_funct(d.totalCount);
 			}) 	 
 			.style("fill", function(d) {
 				
// 				console.log(( d.itemId===current_item_id)? "#00cc33": "#009fff");
...
//                	   console.log(d3.select(this).data());
                	   ...
                	   if (d3.select(this).data()[0].itemId==current_item_data.itemId){
                		   ...
                		   draw_modal_info_svg_tool(d3.select(this),d3.mouse(this),current_item_data);
                		   
                	   }
                	   else{
                		   draw_modal_info_svg_tool(d3.select(this),d3.mouse(this),item_info_map[d3.select(this).data()[0].itemId]);
