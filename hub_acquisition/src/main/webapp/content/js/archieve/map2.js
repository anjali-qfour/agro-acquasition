...
function map_indicator_barchart(svg_elem,indicator_data)
{
	console.log("map_indicator_barchart1 - ");
	
	var dim = d3.select(svg_elem).node().getClientRects()[0];
	var map_dim = d3.select(".map_svg").node().getClientRects()[0];
...
//	console.log(world_indicator_g.node().parentNode);
	...
	var barchart_svg = d3.select(".map_svg")
							.append("g")
							.attr("class","world_indicator_barchart")
							.attr("transform", "translate(" + ( dim.x - map_dim.x - (barchart_w/2) + (dim.width/2) + 5  )+ "," + (dim.y-barchart_h-hu-hz_gap_tn) + ")");
...
//    	console.log(world_indicator_g);
    	map_indicator_barchart(".world_indicator",dataset);
...
//	console.log("draw_no_location_indicator - ");
    ...
    $.get("content/svg/no_location.svg", function(text_data){
    	...
    	map_indicator_barchart(".no_location_indicator",dataset);
...
//	console.log(d3.select(svg_elem));
	...
	function draw_main_map([topoMain,topoKashmir,country_data,reshape_location_data]) 
	{
        ...
        var projection = d3.geoMercator()
				            .center([90, 23])
				            .scale(scale)					            
				            .translate([dim.width / 2, dim.height /2+vu])
				            ;
