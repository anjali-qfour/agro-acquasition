...
function print_label_btn_click2()
{
	 js_doc = new jsPDF();
	
	var js_doc_width = js_doc.internal.pageSize.width -20;
	
	
	consignment_label_html = $('<svg id="barcode" class="barcode_svg"></svg>')
								.appendTo("body");
	
	JsBarcode("#barcode", consignment_id, {
	...
	var svg_str = serializer.serializeToString(consignment_label_html[0]);
	
	
	var $canvas = $('<canvas/>');
	$canvas.attr('width', '300px');
//	$canvas.attr('class', 'barcode_svg');
	$canvas.appendTo('body');
	
	
	canvg($canvas.get(0), svg_str);
...
//													console.log(barcode_DataURL);
								
//								 					js_doc.addImage(barcode_DataURL, 'PNG', 10, 10, js_doc_width, 40);
...
function print_label_btn_click1()
{
	var printDoc = new jsPDF();
	
	
	
//	 consignment_label_html = $('<div> '+
//								'	<svg id="barcode"></svg>'+ 
//								'	<p> Barcode label </p> '+
//								'	<div id="editor"></div>'+
//								'	</div>'
...
//								.appendTo("body");
	...
	consignment_label_html = $('	<svg id="barcode"></svg>' 
								)
								.appendTo("body");
	...
	printDoc.fromHTML(consignment_label_html[0].outerHTML,
					15, 15, {
                    'width': 100,
                    'elementHandlers': specialElementHandlers
                });
	...
	window.open(printDoc.output('bloburi'))
...
function save_consignment_btn_click()
{
	...
	promises.push(validate_select("#product_rate_input",true));
...
function add_consignment()
{
	...
	$.ajax({
	    ...
	    data:  JSON.stringify({ 
			...
			"product_rate" : $("#product_rate_input").datepicker('getDate'),
